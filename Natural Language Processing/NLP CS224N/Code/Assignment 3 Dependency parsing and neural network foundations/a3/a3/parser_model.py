...
class ParserModel(nn.Module):
    ...
    """
    def __init__(self, embeddings: nn.Embedding, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(
            in_features= self.embed_size * self.n_features, out_features=self.hidden_size
        )
        ...
        self.hidden_to_logits = nn.Linear(
            in_features=self.hidden_size, out_features=self.n_classes
        )
    ...
    def forward(self, t):
        ...
        hidden_activation = F.relu(hidden)
        thin_net = self.dropout(hidden_activation)
