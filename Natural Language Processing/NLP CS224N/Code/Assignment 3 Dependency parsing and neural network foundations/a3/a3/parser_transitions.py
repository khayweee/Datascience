...
class PartialParse(object):
    ...
    def parse_step(self, transition: str) -> None:
    ...
    def parse(self, transitions: List[str]) -> List[Tuple[str,str]]:
...
def minibatch_parse(sentences: List[List[str]], model, batch_size: int) -> List[List[Tuple[str, str]]]:
    ...
    unifinished_parses = partial_parses[:]
    while unifinished_parses:
        # Take first batch_size parses in unfinished_parses as the minibatch
        minibatch = unifinished_parses[:batch_size]
        # use the monel to predict the next transition for each partial parse in the minibatch
        transitions = model.predict(minibatch)
        # perform parse step for each partial_parse in minibatch with their predicted transition
        for transition, partial_parse in zip(transitions, minibatch):
            partial_parse.parse_step(transition)
        # remove completed parses
        unifinished_parses[:] = [
            p for p in unifinished_parses if not p.is_completed()
        ]
